...
class SongHandler(AbletonOSCHandler):
    ...
    def init_api(self):
        ...
        #--------------------------------------------------------------------------------
        # Master (Main) track — `song.master_track` lives OUTSIDE `song.tracks`, so the
        # regular track/device handlers can't address it. These endpoints expose the
        # Main track for mastering: load audio effects (Glue Compressor, Limiter, EQ),
